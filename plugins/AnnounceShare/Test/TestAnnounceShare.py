...
class TestAnnounceShare:
    def testAnnounceList(self, file_server):
        ...
        tracker_storage.onTrackerFound("epix://%s:10042" % file_server.ip)
        assert peer.request("getTrackers")["trackers"] == []

        # It needs to have at least one successfull announce to be shared to other peers
        tracker_storage.onTrackerSuccess("epix://%s:10042" % file_server.ip, 1.0)
        assert peer.request("getTrackers")["trackers"] == ["epix://%s:10042" % file_server.ip]
